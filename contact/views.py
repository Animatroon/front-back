...
class MessageViewSet(viewsets.ModelViewSet):
    ...
    def add_message(cls, request):
        serializer = MessageSerializer(data=request.data)
        if serializer.is_valid():
            message = serializer.save() # в этом message хранятся имя message.name почта message.email сообщение само message.message и номер message.telephone
            send_mail('Тема', 'Тело письма', settings.EMAIL_HOST_USER, [message.email]) # Вот функция отправки сообщений на почту
